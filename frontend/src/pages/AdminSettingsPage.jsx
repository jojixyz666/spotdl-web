...
import { Sliders, Save, Loader2, Users, Download, Shield } from 'lucide-react'

export default function AdminSettingsPage() {
  const toast = useToast()
  const [config, setConfig] = useState({ batch_limit: 500, max_concurrent_downloads: 5, require_approval: true })
    ...
    <div className="max-w-lg space-y-6">
      ...
      <form onSubmit={save} className="space-y-5">
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className="card p-6">
          ...
          <p className="text-xs text-text-muted mt-2">How many tracks a user can download at once. Default: 50</p>
        </motion.div>
